...
const { reroll_traitors, rollMap } = require( '../../diceRoller.js' );
...
module.exports = {
	...
	data: new SlashCommandBuilder()
		.setName( 'call_on' )
		.setDescription( 'Use a call-on trait to reroll all traitor dice! Usable once per roll.' )
	,

	async execute( interaction ) 
	{
		let msg = '';
		let prevPool = rollMap.get( interaction.user.id );

		if ( !prevPool )
			{	msg += 'You need to actually make a roll first.';	 }
		else if ( prevPool.isCalledOn )
			{	msg += `You've already called on a trait for this roll.`;	}
		else
		{
			...
			result = reroll_traitors( prevPool.open_ended_pool, prevShade, true )
			expoTally += result[0]
			prevPool.open_ended_pool = result[1];

			// Check exponent Pool ( 1D Array )
			result = reroll_traitors( prevPool.unified_pool, prevShade, prevPool.isOpenEnded )
			expoTally += result[0]
			...
			{
				result = reroll_traitors( helper, prevShade, prevPool.isOpenEnded )
				expoTally += result[0];
			...
			{
				prevPool.isCalledOn = true;
				
				prevPool.unified_gains += expoTally;
				rollMap.set( interaction.user.id, prevPool );
